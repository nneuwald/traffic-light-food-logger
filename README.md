...
| `tests/` | Test harness: six suites covering the classification engine, the USDA mapper, and the lookup flow |
...
With Node.js installed, run `bash tests/extract_and_test.sh`. It re-extracts the JavaScript from the HTML and runs six suites against offline fixtures: 237 hand-written classification and portion checks, 11 USDA mapping checks, 39 assertions over a saved corpus of real category vocabulary, 17 over saved USDA generic-food responses, 23 checks of the name-search plumbing (request shape and ranking), and 15 of the lookup flow itself (what a barcode miss says, how results are ordered, how a USDA outage reads).
